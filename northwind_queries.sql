...
-- List all products with their corresponding suppliers.
...
-- Calculate the average order value (total amount) for each customer
 
select c.customerid , c.customername , avg(od.quantity * p.price) as total_amnt
from customers c 
join orders o on c.CustomerID = o.CustomerID
join orderdetails od on o.OrderID = od.OrderID
join products p on od.ProductID = p.ProductID
...
select  c.customerid , c.customername , o.orderid  , p.productid , p.productname ,od.quantity,p.unit, p.price
...
select e.employeeid , concat(firstname ," ",lastname) as emp_name , sum(od.quantity * p.Price) as total_revenue
from orders o join employees e on o.employeeID = e.EmployeeID
join orderdetails od on o.OrderID = od.OrderID
join products p on od.ProductID = p.ProductID
group by employeeid , concat(firstname ," ",lastname);

-- List all products with their corresponding categories and suppliers.
...
select c.customerid , c.customername , avg(od.quantity) as  avg_order_quantity
from customers c
join orders o on c.CustomerID = o.CustomerID
join orderdetails od on o.orderid  = od.OrderID
group by c.customerid , c.customername ;

-- List all customers who have placed orders in the year 1996.
...
-- Calculate the total quantity of each product sold by a specific employee.

SELECT employees.employeeid , concat(firstname ," ",lastname) as emp_name , products.productname,
SUM(Orderdetails.Quantity) AS TotalQuantity
FROM Products
JOIN OrderDetails ON Products.ProductID = OrderDetails.ProductID
JOIN Orders ON OrderDetails.OrderID = Orders.OrderID
join employees on orders.EmployeeID = employees.EmployeeID
GROUP BY employees.employeeid , concat(firstname ," ",lastname),products.productname  ;

-- List all orders that contain a specific product.

SELECT Orders.OrderID, Customers.CustomerName , products.productname , OrderDetails.Quantity, products.Price
FROM Orders
...
select c.categoryid , c.categoryname , avg(price) as avg_price
from products p
join categories c on p.CategoryID = c.CategoryID
group by c.categoryid , c.categoryname;
